...
Begin VB.Form frmNotePad 
   ...
   Begin VB.Menu mnuTools 
      Caption         =   "&Tools"
      Begin VB.Menu mnuToolsReverse 
         Caption         =   "&Reverse"
      End
      Begin VB.Menu mnuToolsReverseComparison 
         Caption         =   "Reverse Comparison"
      End
   End
...
'** For CC Phase 2/3 ** Added to project
Private Sub mnuToolsReverse_Click()
    Dim sBuffer As String
    sBuffer = Me.Text1.Text
    FastBStringReverse sBuffer
    Me.Text1.Text = sBuffer
End Sub

'** For CC Phase 2/3 ** Added to project
Private Sub mnuToolsReverseComparison_Click()
    Dim sBuffer As String, timerStart As Single
    Dim timeFast As Single, timeVM As Single, timeBasic As Single
    Dim sResponse As String
    Dim lngNumberOfTimesToReverse As Long
    Dim xReversal As Long
    sResponse = InputBox("Number of times to reverse this string: ", "Do a Comparison of VM Reverse to FastReverse", 10000)
    If Not IsNumeric(sResponse) Then Exit Sub
    lngNumberOfTimesToReverse = CStr(sResponse)
    If lngNumberOfTimesToReverse < 1 Then Exit Sub
    sBuffer = Me.Text1.Text
    timerStart = Timer
    For xReversal = 1 To lngNumberOfTimesToReverse
        BasicReverse sBuffer
    Next
    timeBasic = Timer - timerStart
    timerStart = Timer
    For xReversal = 1 To lngNumberOfTimesToReverse
        sBuffer = StrReverse(sBuffer)
    Next
    timeVM = Timer - timerStart
    timerStart = Timer
    For xReversal = 1 To lngNumberOfTimesToReverse
        FastBStringReverse sBuffer
    Next
    timeFast = Timer - timerStart
    MsgBox "To reverse this " & Len(sBuffer) & " character string " & lngNumberOfTimesToReverse & " times took: " & vbNewLine & vbNewLine & "FastBStrReverse: " & timeFast & " seconds" & vbNewLine & "virtual machine: " & timeVM & " seconds" & vbNewLine & "basic sub: " & timeBasic & " seconds"
End Sub

Private Sub BasicReverse(sBuffer)
    Dim lenBuffer As Long, idxChar As Long, charSwap As String
    lenBuffer = Len(sBuffer)
    For idxChar = 1 To Fix(lenBuffer / 2)
        charSwap = Mid$(sBuffer, idxChar, 1)
        Mid$(sBuffer, idxChar, 1) = Mid$(sBuffer, lenBuffer - idxChar + 1, 1)
        Mid$(sBuffer, lenBuffer - idxChar + 1, 1) = charSwap
    Next
End Sub
