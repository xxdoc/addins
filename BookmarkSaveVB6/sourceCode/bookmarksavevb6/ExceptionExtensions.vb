...
''' <summary>
''' Generic Exception Handling Dialog
''' </summary>
''' <remarks></remarks>
...
Public Class ExceptionDialog
    ...
    ''' <summary>
    ''' Returns TRUE if the debug button on the exception handling dialog was clicked.
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ...
    ''' <summary>
    ''' Allows user to show the exception dialog without having an exception
    ''' available. Shouldn't normally be used, but can be necessary in special 
    ''' circumstances.
    ''' </summary>
    ...
    Public Overloads Shared Function Show( _
        ...
        '     but pass nothing for the exception itself
        Return ExceptionExtensions.Show(Nothing, WhatHappened, HowUserAffected, WhatUserCanDo, MoreDetails, Buttons, Icon, DefaultButton)
    End Function
...
''' <summary>
''' Provides a host of methods used to extend the normal .NET Exception class.
''' Sets up global exception handling hooks, a generic detailed 
''' unhandled exception dialog, logging and more.
''' </summary>
''' <remarks>This class is primarily intended for use internally. 
''' However, its functions are useful enough that I've gone 
''' ahead and exposed it for use by plugins as well
''' <code>
''' Private Sub CustomMalaPlugin_MaLaGameSelected(ByVal EventGame As MaLaEventGame) Handles Me.MaLaGameSelected
'''     Try
'''         '---- stop any slideshow that might be running
'''         '     and video that might be playing
'''         StopSlideShow()
''' 
'''         '---- gather all available files for this new game
'''         GatherSlideShowFiles()
''' 
'''         '---- get effects to use
'''         GetEffectsList()
''' 
'''         '---- show the first file in rotation
'''         ShowNextInSlideShow()
'''  
'''       Catch ex As Exception
'''           ex.Show()
...
Public Module ExceptionExtensions
    ...
    ''' </summary>
    ''' <remarks>Controls which buttons are visible on the Exception dialog.
    ''' Similiar to the way Msgbox buttons are controlled.</remarks>
    ...
    Private Function ReplaceStringVals(ByVal Ex As Exception, ByVal InputString As String) As String
        ...
        Else
            GetAssemblyAttribs()
            InputString = Replace(InputString, "(filename)", _AppCodeBase, , , CompareMethod.Text)
            InputString = Replace(InputString, "(codebase)", _AppCodeBase, , , CompareMethod.Text)
            InputString = Replace(InputString, "(fullname)", _AppFullName, , , CompareMethod.Text)
            InputString = Replace(InputString, "(company)", _AppCompany, , , CompareMethod.Text)
            InputString = Replace(InputString, "(product)", _AppProduct, , , CompareMethod.Text)
            InputString = Replace(InputString, "(app)", _AppProduct, , , CompareMethod.Text)
            InputString = Replace(InputString, "(contact)", _AppProduct, , , CompareMethod.Text)
            InputString = Replace(InputString, "(exception)", If(Ex Is Nothing, "", Ex.Message), , , CompareMethod.Text)
        End If
        Return InputString
    End Function
    ...
    Private Function GetDefaultMore(ByVal Ex As Exception, ByVal MoreDetails As String) As String
        ...
        With objStringBuilder
            ...
            End If
            .Append(ReplaceStringVals(Ex, "The original exception text was '(exception)'"))
            .Append(Environment.NewLine)
            If Len(MoreDetails) = 0 Then
                .Append(DEFAULTMORE)
                .Append(Environment.NewLine)
    ...
    ''' <value></value>
    ''' <remarks></remarks>
    Public ReadOnly Property IsIDE() As Boolean
        Get
    ...
    Private Sub InstallHandlers()
        ...
        RemoveHandler System.Windows.Forms.Application.ThreadException, AddressOf UnhandledThreadExceptionHandler
        AddHandler System.Windows.Forms.Application.ThreadException, AddressOf UnhandledThreadExceptionHandler
    ...
    Private Function StackFrameToString(ByVal FrameNum As Integer, ByVal sf As StackFrame) As String
        ...
        With sb
            ...
            If mi.DeclaringType IsNot Nothing Then
                MethodName = mi.DeclaringType.Namespace & "." & mi.DeclaringType.Name & "."
            ElseIf mi.ReflectedType IsNot Nothing Then
                MethodName = mi.ReflectedType.Namespace & "." & mi.ReflectedType.Name & "."
            Else
                '---- no type available. Odd, but not much else to do. Might be able to retrieve 
                '     more info from mi.module...
    ...
    Private Sub MapStackFrameToSourceLine(ByVal sf As StackFrame, ByRef Line As Integer, ByRef SourceFile As String)
        ...
        If ILOffset <> StackFrame.OFFSET_UNKNOWN Then
            ...
            For i = alm.AddressToLineMap.Count - 1 To 0 Step -1
                Logger.Log(String.Format("{0}  Addr={1} Search={2}  Line={3}", i, Hex(alm.AddressToLineMap(i).Address), Hex(Addr), alm.AddressToLineMap(i).Line.ToString))
            ...
            If i >= 0 Then
                '---- but we have to have found at least a close match
    ...
    Private Function InternalEnhancedStackTrace(ByVal objException As Exception) As String
        ...
        If objException IsNot Nothing Then
            Dim Trace = New StackTrace(objException, True)
            If Trace.FrameCount > 0 Then
                Return EnhancedStackTrace(Trace)
            End If
        End If

        '---- have to generate a stack trace at this point
    ...
    ''' </summary>
    ''' <returns></returns>
    Public Function EnhancedStackTrace() As String
        Return EnhancedStackTrace(New StackTrace(True), ERRCLASSNAME)
    End Function
    ...
    Private Sub GetAssemblyAttribs()
        ...
        If Len(_AppProduct) = 0 Then
            ...
            _AppProduct = _AppCodeBase
        End If
        If Len(_AppCompany) = 0 Then
            '---- just use the filename
            _AppCompany = _AppCodeBase
        End If
    End Sub
