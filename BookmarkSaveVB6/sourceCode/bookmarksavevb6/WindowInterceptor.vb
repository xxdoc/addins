...
'---------------------------------------------------------------------
'
'Window Creation interception code for BookmarkSave project
'I got the base for these routines from somewhere, but those links are
'long gone. As I recall, it was all C#, which I've converted and
'refactored considerably anyway.
'
...
Public Class WindowInterceptor
    ...
    Private cbf As Win32.Functions.HookProc

    ''' <summary>
    ''' Delegate to process intercepted window
    ''' </summary>
    Public Event ProcessWindow(Sender As Object, e As WindowEventArgs)
    Public Delegate Sub ProcessWindowDelegate(hwnd As IntPtr)
    Private _ProcessWindowDelegate As ProcessWindowDelegate

    Private owner_window As IntPtr = IntPtr.Zero


    ''' <summary>
    ''' Use the event
    ''' </summary>
    ''' <param name="owner_window"></param>
    ''' <remarks></remarks>
    Public Sub New(owner_window As IntPtr)
        Me.New(owner_window, Nothing)
    End Sub
    ...
    ''' <param name="ProcessWindowDelegate">custom delegate to process intercepted window
    ''' </param>
    Public Sub New(owner_window As IntPtr, ProcessWindowDelegate As ProcessWindowDelegate)
        _ProcessWindowDelegate = ProcessWindowDelegate
        Me.owner_window = owner_window

        cbf = New Win32.Functions.HookProc(AddressOf _hook_proc)
        'notice that win32 callback function must be a global variable within class
        'to avoid disposing it!
        _HookID = Win32.Functions.SetWindowsHookEx(Win32.HookType.WH_CALLWNDPROCRET, cbf, IntPtr.Zero, Win32.Functions.GetCurrentThreadId())
        'Debug.Print("Interceptor Set " & _HookID.ToString)
    End Sub
    ...
    Public Sub [Stop]()
        If _HookID <> IntPtr.Zero Then
            Win32.Functions.UnhookWindowsHookEx(_HookID)
        End If
        _HookID = IntPtr.Zero
        'Debug.Print("Hook stopped")
    End Sub
    ...
    Private Function _hook_proc(nCode As Integer, wParam As IntPtr, lParam As IntPtr) As IntPtr
        If nCode < 0 Then
            Return Win32.Functions.CallNextHookEx(_HookID, nCode, wParam, lParam)
        End If

        Dim msg As Win32.CWPRETSTRUCT = DirectCast(Marshal.PtrToStructure(lParam, GetType(Win32.CWPRETSTRUCT)), Win32.CWPRETSTRUCT)

        'filter out create window events only
        Select Case msg.message
            Case Win32.Messages.WM_SHOWWINDOW, Win32.Messages.WM_ACTIVATE, Win32.Messages.WM_CREATE
                Dim bCaught = False
                If owner_window <> IntPtr.Zero Then
                    Dim h As Integer = Win32.Functions.GetWindow(msg.hwnd, Win32.Functions.GW_OWNER)
                    If owner_window = New IntPtr(h) Then bCaught = True
                Else
                    bCaught = True
                End If
                'check if owner is that is specified
                If bCaught Then
                    '---- depending on which is set, invoke the callback or the event
                    If _ProcessWindowDelegate IsNot Nothing Then
                        _ProcessWindowDelegate.Invoke(msg.hwnd)
                    Else
                        RaiseEvent ProcessWindow(Me, New WindowEventArgs(msg.hwnd))
                    End If
                End If
            Case Else
        End Select
...
Namespace Win32
    Public NotInheritable Class Functions
        Private Sub New()
        End Sub
        Public Delegate Function EnumProc(hwnd As IntPtr, lParam As Integer) As Boolean
        ...
        Public Shared Function SetWindowsHookEx(hook As HookType, callback As HookProc, hMod As IntPtr, dwThreadId As UInteger) As IntPtr
        End Function
        ...
        <DllImport("user32.dll")> _
        Public Shared Function UnhookWindowsHookEx(hhk As IntPtr) As IntPtr
        End Function
        ...
        <DllImport("user32.dll")> _
        Public Shared Function DefDlgProc(hDlg As IntPtr, Msg As UInteger, wParam As IntPtr, lParam As IntPtr) As IntPtr
        End Function
        ...
        <DllImport("user32.dll")> _
        Public Shared Function GetWindowTextLength(hWnd As IntPtr) As IntPtr
        End Function
        ...
        <DllImport("user32.dll")> _
        Public Shared Function SetWindowText(hwnd As IntPtr, str As String) As Integer
        End Function
        ...
        <DllImport("user32.dll")> _
        Public Shared Function InternalGetWindowText(hwnd As IntPtr, s As StringBuilder, nMaxCount As Integer) As Integer
        End Function

        <DllImport("User32.Dll")> _
        Public Shared Sub GetClassName(hwnd As IntPtr, s As StringBuilder, nMaxCount As Integer)
        End Sub
        ...
        Public Shared Function FindWindowEx(parent_h As IntPtr, child_h As IntPtr, lpClassName As String, lpWindowName As String) As Integer
        End Function

        <DllImport("user32.dll", EntryPoint:="GetWindow")> _
        Public Shared Function GetWindow(hwnd As IntPtr, flag As Integer) As Integer
        End Function
        ...
        Public Shared Function SendMessage(hwnd As IntPtr, Msg As UInteger, wParam As Integer, lParam As Integer) As Integer
        End Function

        <DllImport("User32.dll", EntryPoint:="PostMessage")> _
        Public Shared Function PostMessage(hwnd As IntPtr, Msg As UInteger, wParam As Integer, lParam As Integer) As Integer
        End Function
        ...
        <DllImport("user32")> _
        Public Shared Function EnumWindows(cbf As EnumProc, lParam As Integer) As Integer
        End Function

        <DllImport("user32")> _
        Public Shared Function EnumChildWindows(hwnd As IntPtr, cbf As EnumProc, lParam As Integer) As Integer
        End Function
        ...
        <DllImport("User32.dll", EntryPoint:="EnumThreadWindows")> _
        Public Shared Function EnumThreadWindows(dwThreadId As UInteger, cbf As EnumProc, lParam As Integer) As Boolean
        End Function
    End Class
...
Namespace Win32
    Public Enum Messages As UInteger
        WM_NULL = &H0
        WM_CREATE = &H1
        WM_DESTROY = &H2
        WM_MOVE = &H3
